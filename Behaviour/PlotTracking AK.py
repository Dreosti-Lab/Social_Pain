...
# Set folder
Heat  = base_path + r'/Folderlist_New.txt'

groups, ages, folderNames, fishStatus = SPU.read_folder_list(Heat)
...
for idx,folder in enumerate(folderNames):
    ...
    for i in range(0,6):
     ...
     # Extract tracking data (NS)
        ...
        # SPM Score
        SPM_Heat = np.mean(fx_S[good_frame_S])- np.mean(fx_NS[good_frame_NS])
        Gradient = np.mean(SPM_Heat)
        print(Gradient)
        
Control = base_path + r'/Experiment_10/FolderList/Exp_10.txt'

groups, ages, folderNames, fishStatus = SPU.read_folder_list(Control)
...
for idx,folder in enumerate(folderNames):
    ...
    for i in range(0,6):
     ...
     # Extract tracking data (NS)
        ...
        # SPM Score
        SPM_Control = np.mean(fx_S[good_frame_S])- np.mean(fx_NS[good_frame_NS])
        No_Gradient = np.mean(SPM_Control)
        print(No_Gradient)
        
      
        # plt.hist(x, bins = 30, color='blue')
        # plt.title('SPM for each Fish (n=42)')
        # plt.xlabel('SPM score (px)')
