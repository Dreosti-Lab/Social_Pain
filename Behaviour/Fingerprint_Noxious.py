...
conditionNames.append(r'Heat_36°C')
conditionNames.append(r'AITC_200uM')
...
dfAll=pd.DataFrame(AllFish,columns=['Condition','BoutsPerSecond','DistPerBout','Speed', 'Turn','InterBout', 'Freezes'])
...
fingerprint =plt.figure(figsize= (16,6))

df_MeanZ_exBas=df_MeanZ.drop('Baseline')

ax=sns.heatmap(df_MeanZ_exBas,vmin=-4,vmax=4 ,cmap= sns.diverging_palette(220, 20, as_cmap=True), linewidth=0.5, square = True)#, cbar_kws={'shrink':0.8})
...
df_pivot = df_pivot[['BoutsPerSecond', 'DistPerBout','Speed','Turn', 'InterBout','Freezes']]
...
fingerprint.savefig(base_path + '/Figure_Nox/test/fingerprint.eps', format='eps', dpi=300,bbox_inches= 'tight')           
