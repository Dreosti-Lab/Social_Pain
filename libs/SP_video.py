...
# Process Video : Track fish in AVI
def improved_fish_tracking2(input_folder, output_folder, ROIs):
    ...
    for f in range(0,numFrames):  
        ...
        for i in range(0,6):
            ...
            for values in diff():
                if diff > 0:
                     threshold_level = np.median(background_ROIs[i])/5
              # finds body + eye  
                else:
                     threshold_level = np.median(background_ROIs[i])/3
            # finds only eyes
           
            # Determine current threshold (Sensitivity : detected range of pixel change)
            # How different does it need to be for it to be picked up as the fish, lower threshold if you want to track fish that are not moving much 
            # we tried a few and thought 4 or 5 is best, fish gave more of the fish hence why we chose this. 
            # bodyThreshold_level = np.median(background_ROIs[i])/5
            # eyeThreshold_level = np.median(background_ROIs[i])/3        
            ...
            ##################################################################################
                    ...
                    if (f != 0):
                        ...
                        if diff > 0:
                            diff[diff < threshold_level] = 0
                            totalDiff = np.sum(diff)
                            frame_by_frame_diff = (np.float32(previous_ROIs[i]) - np.float32(crop)) / 2 # Adjust for increases and decreases across frames
                            frame_by_frame_diff[frame_by_frame_diff < threshold_level] = 0
                            motion = np.sum((frame_by_frame_diff))/totalDiff
                        else: 
                            
                            diff[diff < threshold_level] = 1
                            totalDiff = np.sum(diff)
                            frame_by_frame_diff = (np.float32(previous_ROIs[i]) - np.float32(crop)) / 2 # Adjust for increases and decreases across frames
                            frame_by_frame_diff[frame_by_frame_diff < threshold_level] = 1
                            motion = np.sum((frame_by_frame_diff))/totalDiff
                    else:
                        motion = 0
                    # Save Masked Fish Image from ROI (for subsequent frames motion calculation)
